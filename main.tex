...
%% article.
\begin{abstract}
Exploring and visualising a complex model in a single view can be challenging as it can require considerable amount of computational resources and make the model difficult to comprehend due to information overload.
Picto Web is a tool designed for complex model exploration. Using the Epsilon Generation Language for model-to-text transformation, it transforms domain-specific models into transient web-based views in different formats, such as HTML, Graphviz, and PlantUML. 
Picto Web uses caching to reduce the response time of view requests and incremental model-to-text transformation to regenerate views efficiently. 
...
\begin{figure}
  ...
  \caption{Picto plugin for Eclipse environment.\agd{would it be more readable to crop the figure to just the .picto editor, the .model editor, and the Picto views? You don't need the Properties view.}}
