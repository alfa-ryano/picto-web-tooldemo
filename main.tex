...
%% article.
\begin{abstract}
Exploring and visualizing a complex model in a single view can be very challenging as it requires a lot of computational resources and can lead to information overload, making the model difficult to comprehend.
Picto Web is a tool designed for complex model exploration. It takes domain-specific models as inputs to generate transient web-based views in different formats (e.g., HTML, Graphviz, PlantUML) using model-to-text transformations in Epsilon Generation Language. 
Moreover, Picto Web uses caching to reduce the response time of views requests and incremental model-to-text transformation to regenerate views efficiently. 
Also, it supports push notifications to clients when views are updated and is packaged as a Docker container for ease of use.
\end{abstract}
